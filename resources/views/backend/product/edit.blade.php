...
<div class="row">
    <div class="col-lg-offset-1 col-md-10">
        <form action="{{ route('product.update',$product->id) }}" enctype="multipart/form-data" class="form form-validate" method="POST">
            @csrf
            @method('PATCH')
            <div class="card">
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group  {{$errors->has('size') ? 'has-error' : ''}}">
                                <input type="text" class="form-control" name="size" id="size" value="{{$product->size}}"> @if($errors->has('days'))
                    ...
                    <div class="row">
                        <div class="form-group">
                            <div class="col-sm-12">
                                ...
                                <select class="form-control select2-multi   {{$errors->has('title') ? 'has-error' : ''}}" name="materials[]"
                                    multiple="multiple">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            <div class="form-group {{$errors->has('mtitle') ? 'has-error' : ''}} ">
                                <input type="text" name="mtitle" id="mtitle" class="form-control" value="{{$product->mtitle}}" required>                                @if($errors->has('mtitle'))
                        ...
                        <div class="col-sm-12" id="desc">
                            <div class="form-group  {{$errors->has('description') ? 'has-error' : ''}} ">
                                <textarea class="form-control meta" maxlength="160" name="description" id="description {{$errors->has('description')? 'inputError' : ''}}"
                                    required>{{$product->meta_description}}</textarea> @if ($errors->has('description'))
...
<script>
	...
	$('.select2-multi').select2().val({{ json_encode($product->material()->allRelatedIds() )}}).trigger('change');
</script>
@stop